  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className="flex items-center gap-2 px-[var(--spacing-md)] py-[var(--spacing-sm)] border-[var(--border-width-normal)] border-[var(--color-border)] rounded-[var(--border-radius-md)] bg-[var(--color-background)] text-[var(--color-text-primary)] hover:border-[var(--color-text-secondary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent transition-colors"
      >
      ...
      {showDropDown && (
        <div className="absolute z-50 top-full mt-1 right-0 w-48 bg-[var(--color-background)] border-[var(--border-width-normal)] border-[var(--color-border)] rounded-[var(--border-radius-md)] shadow-[var(--shadow-lg)] overflow-hidden">
          <div className="py-1">
            {options.map((option) => (
              <button
                key={option.value}
                onClick={() => handleSortSelect(option)}
                className={`w-full flex items-center gap-3 px-4 py-2 text-left text-[var(--font-size-sm)] hover:bg-[var(--color-surface)] transition-colors ${
